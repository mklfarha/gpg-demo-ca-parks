...
class RecreationArea extends React.Component {
    constructor(props) {
        super(props)        
        this.state = { 
            pending:true,           
    ...
    componentDidMount() {        
        ...
        var cachedColumns = localStorage.getItem("recreation_area_columns")     
    ...
    fetchItems() {        
        ...
        Client
            .query({
                query: gql`
                query RecreationAreaByStatus($limit: Int, $offset: Int) {
                    RecreationAreaByStatus(status:"enabled",limit:$limit, offset:$offset){ 
                        id,
                        name,
                        status,
                        created_at,
            ...
            .then((result) => {
                ...
                const items = result.data.RecreationAreaByStatus
    ...
    searchItems() {        
        ...
        Client
            ...
            .then((result) => {
                ...
                const items = result.data.SearchRecreationArea
                if (items.length < this.state.pageSize){
                    this.state.total = ((this.state.page)*this.state.pageSize)+items.length
                }                
                Promise.allSettled(EntityRefPromises(items)).then(()=>{
        ...
        return (
            <div>
                ...
                <div className="actionsBar">                     
                                    ...
                                    this.props.popups.open(GPGModal, {
                                                ...
                                                content: <Columns 
                                                    ...
                                                    onChange={ (columns) => {
                                                        ...
                                                        localStorage.setItem("recreation_area_columns", JSON.stringify(omit))
                                                        this.setState(this.state)
                                                    } }
                                                 />,
                                                onClose: (...params) => {
                    ...
                    <Link to="/recreation_area/edit"><MdCreate /> Create</Link>                                                         
                </div>
                ...
                <div className="tableWrapper" style={{borderTopColor:StringToColour('recreation_area')}}>                                                             
                <DataTable
                    title="RecreationArea"
                    ...
                    onRowClicked={(row) => {                             
                            if (pressedKeys[91]) {
                                var url = "/recreation_area/edit/"+row.id;
                                window.open(url, '_blank', 'noopener,noreferrer').focus();
                            } else {
                                window.location.href = "/recreation_area/edit/"+row.id;                           
                            }
                        }
                    }
                />
...
export default (props) => <RecreationArea {...props}
